...
public interface ElasticsearchRel extends RelNode {
  ...
  class Implementor {
    ...
    /**
     * Projections (aka Source filtering in ES)
     * @see <a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/search-request-source-filtering.html">Source filtering</a>
     */
    ...
    /**
     * Expression(s) which contain star ({@code *}) pseudo column
     * like {@code select * ... } or {@code select count(*) ...}. This is used in ES query
     * building and result parsing.
     */
    final List<String> starExpression = new ArrayList<>();

    /**
     * Allows to bucket documents together. Similar to {@code select ... from table group by field1}
    ...
    void addStar(String expression) {
      Objects.requireNonNull(expression, "expression");
      starExpression.add(expression);
    }
