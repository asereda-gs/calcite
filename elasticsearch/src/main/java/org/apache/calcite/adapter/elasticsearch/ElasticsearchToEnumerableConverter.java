...
public class ElasticsearchToEnumerableConverter extends ConverterImpl implements EnumerableRel {
  ...
  private static List<Map.Entry<String, RelFieldCollation.Direction>> toListMapPairs(Map<String, RelFieldCollation.Direction> map) {
    List<Map.Entry<String, RelFieldCollation.Direction>> selectList = new ArrayList<>();
    for (Map.Entry<String, RelFieldCollation.Direction> entry : Pair.zip(map.keySet(), map.values())) {
